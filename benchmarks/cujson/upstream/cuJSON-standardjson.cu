...
#ifdef CUJSON_STRUCTURAL_BITMAP_TIMER
static float cujson_structural_validation_ms = 0.0f;
static float cujson_structural_tokenizer_ms = 0.0f;
#endif
...
#ifdef CUJSON_STRUCTURAL_BITMAP_TIMER
    cudaEvent_t structural_start, structural_stop;
    cudaEventCreate(&structural_start);
    cudaEventCreate(&structural_stop);
    float structural_step_ms = 0.0f;
#endif


    // _________________PART_1_______________________
#ifdef CUJSON_STRUCTURAL_BITMAP_TIMER
    cudaEventRecord(structural_start, 0);
#endif
    checkAscii<<<numBlock_16B, BLOCKSIZE>>>(block_GPU, size, total_padded_16B, hastUTF8_GPU, WORDS);
#ifdef CUJSON_STRUCTURAL_BITMAP_TIMER
    cudaEventRecord(structural_stop, 0);
#endif
    cudaStreamSynchronize(0);
#ifdef CUJSON_STRUCTURAL_BITMAP_TIMER
    cudaEventElapsedTime(&structural_step_ms, structural_start, structural_stop);
    cujson_structural_validation_ms += structural_step_ms;
#endif
...
#ifdef CUJSON_STRUCTURAL_BITMAP_TIMER
        cudaEventDestroy(structural_start);
        cudaEventDestroy(structural_stop);
#endif
...
inline uint8_t * Tokenize(  uint8_t* block_GPU, 
    ...
    cudaEventCreate(&structural_stop);
    float structural_step_ms = 0.0f;
    float structural_tokenizer_ms = 0.0f;
#endif
...
#ifdef CUJSON_STRUCTURAL_BITMAP_TIMER
    cudaEventElapsedTime(&structural_step_ms, structural_start, structural_stop);
    structural_tokenizer_ms += structural_step_ms;
#endif
...
#ifdef CUJSON_STRUCTURAL_BITMAP_TIMER
    cudaEventRecord(structural_stop, 0);
    cudaEventSynchronize(structural_stop);
    cudaEventElapsedTime(&structural_step_ms, structural_start, structural_stop);
    structural_tokenizer_ms += structural_step_ms;
#endif
    // cudaEventRecord(stop, 0);
    ...
    cudaEventElapsedTime(&structural_step_ms, structural_start, structural_stop);
    structural_tokenizer_ms += structural_step_ms;
    cujson_structural_tokenizer_ms += structural_tokenizer_ms;
    cudaEventDestroy(structural_start);
    cudaEventDestroy(structural_stop);
    return reinterpret_cast<uint8_t*>(general_ptr);
#endif
    // cudaEventRecord(stop, 0);
    // cudaEventSynchronize(stop);
    // cudaEventElapsedTime(&milliseconds, start, stop);
...
#ifdef CUJSON_STRUCTURAL_BITMAP_TIMER
    constexpr int structural_warmups = 5;
    constexpr int structural_runs = 10;
    uint8_t* structural_workspace = nullptr;
    uint32_t structural_last_index = 0u;
    uint32_t open_close_last_index = 0u;
    int structural_return_size = 0;
    uint32_t* structural_indices = nullptr;
    uint32_t* open_close_indices = nullptr;

    for (int run = 0; run < structural_warmups; ++run) {
        UTF8Validation(reinterpret_cast<uint32_t*>(block_GPU), size_32);
        structural_workspace = Tokenize(block_GPU, size, structural_return_size,
            structural_last_index, open_close_last_index, structural_indices,
            open_close_indices, lastStructuralIndex, lastChunkIndex);
        cudaFree(structural_workspace);
    }
    cujson_structural_validation_ms = 0.0f;
    cujson_structural_tokenizer_ms = 0.0f;
    for (int run = 0; run < structural_runs; ++run) {
        UTF8Validation(reinterpret_cast<uint32_t*>(block_GPU), size_32);
        structural_workspace = Tokenize(block_GPU, size, structural_return_size,
            structural_last_index, open_close_last_index, structural_indices,
            open_close_indices, lastStructuralIndex, lastChunkIndex);
        if (run + 1 != structural_runs) cudaFree(structural_workspace);
    }

    uint32_t structural_words = static_cast<uint32_t>((size + 31u) / 32u);
    std::vector<uint32_t> output_host(structural_words * 4u);
    cudaMemcpy(output_host.data(), structural_workspace, output_host.size() * sizeof(uint32_t), cudaMemcpyDeviceToHost);
    uint64_t output_checksum = 0u;
    for (uint32_t value : output_host) output_checksum = output_checksum * 1315423911ull + value;
    printf("method=cujson_upstream_baseline validation_us=%.3f tokenizer_us=%.3f avg_us=%.3f checksum=%" PRIu64 " utf_error=0 block_size=%d runs=%d\n",
        cujson_structural_validation_ms * 1000.0f / structural_runs,
        cujson_structural_tokenizer_ms * 1000.0f / structural_runs,
        (cujson_structural_validation_ms + cujson_structural_tokenizer_ms) * 1000.0f / structural_runs,
        output_checksum, BLOCKSIZE, structural_runs);
    cudaFree(structural_workspace);
    fflush(stdout);
    exit(0);
#endif
