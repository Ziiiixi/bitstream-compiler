...
// Changing the incoming slash state can only toggle the first quote immediately
// following the word's leading slash run. F1 stores that position in one byte;
// 32 means that the two incoming-state candidates are identical.
__global__ void compactEnumerateSlashCandidatesKernel(const uint8_t* input, uint64_t input_size,
    uint32_t* operator_bitmap, uint32_t* raw_open_close,
    uint32_t* structural_if_slash_zero, uint8_t* trailing_slashes,
    uint8_t* first_state_sensitive_quote, uint32_t* quote_parity_if_slash_zero,
    uint32_t* utf_error) {
    ...
    uint32_t leading_slashes = slash_word == 0xffffffffu ? 32u :
        static_cast<uint32_t>(__ffs(~slash_word) - 1);
    uint32_t sensitive_quote = leading_slashes < 32u &&
        (current_quote & (1u << leading_slashes)) != 0u ? leading_slashes : 32u;
    ...
    first_state_sensitive_quote[word_id] = static_cast<uint8_t>(sensitive_quote);
...
// F2 resolves the incoming slash state. Selecting E=1 is an XOR with the
// operator suffix beginning at the one state-sensitive quote; quote parity
// toggles for exactly the same words. There is no input reread or recovery.
__global__ void compactResolveAndSelectSlashCandidateKernel(const uint8_t* trailing_slashes,
    const uint8_t* first_state_sensitive_quote, uint32_t word_count,
    const uint32_t* operator_bitmap, uint32_t* structural_if_slash_zero,
    uint32_t* quote_parity_if_slash_zero) {
    uint32_t word_id = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    uint32_t sensitive_quote = word_id < word_count ?
        static_cast<uint32_t>(first_state_sensitive_quote[word_id]) : 32u;
    bool select_state_one = incoming_slash != 0u && sensitive_quote < 32u;
    if (select_state_one) {
        uint32_t suffix_mask = 0xffffffffu << sensitive_quote;
        structural_if_slash_zero[word_id] ^=
            operator_bitmap[word_id] & suffix_mask;
    }

    uint32_t parity_toggle_mask = __ballot_sync(0xffffffffu, select_state_one);
    uint32_t lane = threadIdx.x & 31u;
    if (lane == 0u) {
        uint32_t warp_id = word_id >> 5u;
        quote_parity_if_slash_zero[warp_id] ^= parity_toggle_mask;
    }
}
...
struct CompactHierarchicalEnumerationInput {
    std::string path;
    ...
    uint8_t* first_state_sensitive_quote = nullptr;
...
static cudaError_t launchCompactHierarchicalEnumeration(CompactHierarchicalEnumerationInput& input) {
    ...
    compactEnumerateSlashCandidatesKernel<<<input.blocks, FULL_THREADS, 0, input.stream>>>(input.input,
        input.input_size, input.operators, input.raw_open_close, input.structural,
        input.trailing_slashes, input.first_state_sensitive_quote,
        input.quote_parity_masks, input.utf_error);
    compactResolveAndSelectSlashCandidateKernel<<<input.blocks, FULL_THREADS, 0, input.stream>>>(
        input.trailing_slashes, input.first_state_sensitive_quote, input.word_count,
...
int main(int argc, char** argv) {
    ...
    std::vector<CompactHierarchicalEnumerationInput> inputs(static_cast<size_t>(argc - 2));
    uint64_t total_blocks = 0u;
    for (int argument = 2; argument < argc; ++argument) {
        CompactHierarchicalEnumerationInput& input = inputs[static_cast<size_t>(argument - 2)];
        ...
        CUDA_CHECK(cudaMalloc(&input.first_state_sensitive_quote,
                              input.padded_words * sizeof(uint8_t)));
    ...
    for (int warmup = 0; warmup < 3; ++warmup) {
        for (CompactHierarchicalEnumerationInput& input : inputs)
            CUDA_CHECK(launchCompactHierarchicalEnumeration(input));
        CUDA_CHECK(cudaDeviceSynchronize());
    }
    ...
    for (int run = 0; run < runs; ++run) {
        ...
        for (size_t index = 0u; index < inputs.size(); ++index) {
            CompactHierarchicalEnumerationInput& input = inputs[index];
    ...
    std::cout << std::fixed << std::setprecision(3)
              << "method=cujson_hierarchical_enumeration_compact inputs=" << inputs.size()
    ...
    for (CompactHierarchicalEnumerationInput& input : inputs) {
        ...
        CUDA_CHECK(cudaFree(input.first_state_sensitive_quote));
