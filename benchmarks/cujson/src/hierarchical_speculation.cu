...
__global__ void speculateKernel(const uint8_t* input, uint64_t input_size,
    uint32_t* operator_bitmap, uint32_t* raw_open_close, uint32_t* structural_bitmap,
    uint8_t* trailing_slashes, uint32_t* quote_parity_masks, uint32_t* utf_error) {
    ...
    trailing_slashes[word_id] = static_cast<uint8_t>(trailing);
    ...
    structural_bitmap[word_id] = ~in_string & operator_word;
}

// F1 has completed before this kernel starts, so every trailing-slash count is
// visible in global memory. Resolve the incoming slash state and validate the
// state-zero escaped-quote prediction in the same pass. Most words read only
// the immediately preceding count; an all-backslash word (count 32)
// propagates the state and makes the thread continue looking backward.
__global__ void lookbackAndValidateEscapedQuotesKernel(const uint8_t* input,
    const uint8_t* trailing_slashes, uint32_t word_count,
    ...
    if (word_id < word_count && word_id != 0u) {
        ...
        if ((previous_trailing_slashes & 1u) != 0u) {
            uint64_t byte_start = static_cast<uint64_t>(word_id) * 32u;
            uint32_t leading_slashes = 0u;
            while (leading_slashes < 32u && input[byte_start + leading_slashes] == '\\')
                ++leading_slashes;
            needs_recovery = leading_slashes < 32u && input[byte_start + leading_slashes] == '"';
        }
    }

    unsigned recovery_mask = __ballot_sync(0xffffffffu, needs_recovery);
    if (recovery_mask != 0u) {
        uint32_t lane = threadIdx.x & 31u;
        uint32_t leader = static_cast<uint32_t>(__ffs(recovery_mask) - 1);
        uint32_t base = lane == leader ? atomicAdd(recovery_count,
            static_cast<uint32_t>(__popc(recovery_mask))) : 0u;
        base = __shfl_sync(0xffffffffu, base, leader);
        if (needs_recovery) {
            uint32_t lower_lanes = lane == 0u ? 0u : ((1u << lane) - 1u);
            uint32_t rank = static_cast<uint32_t>(__popc(recovery_mask & lower_lanes));
            recovery_words[base + rank] = word_id;
        }
    }
}

__global__ void recoverEscapedQuotesKernel(const uint8_t* input,
    uint64_t input_size, const uint32_t* recovery_words, const uint32_t* recovery_count,
    const uint32_t* operator_bitmap, uint32_t* structural_bitmap,
    uint32_t* quote_parity_masks) {
    uint32_t position = blockIdx.x * blockDim.x + threadIdx.x;
    uint32_t stride = blockDim.x * gridDim.x;
    uint32_t count = *recovery_count;
    for (; position < count; position += stride) {
        uint32_t word_id = recovery_words[position];
        const uint4* input_vectors = reinterpret_cast<const uint4*>(input);
        uint4 first_vector = input_vectors[word_id * 2u];
        uint4 second_vector = input_vectors[word_id * 2u + 1u];
        uint32_t loaded_words[8] = {first_vector.x, first_vector.y, first_vector.z, first_vector.w,
        ...
        uint32_t predicted_escape = current_quote & (slash_word << 1u);
        uint32_t predicted_quote = predicted_escape == 0u ? current_quote :
            fullFilterEscapedQuotesWord(current_quote, slash_word, 0u);
        uint32_t actual_escape = current_quote & ((slash_word << 1u) | 1u);
        uint32_t real_quote = actual_escape == 0u ? current_quote :
            fullFilterEscapedQuotesWord(current_quote, slash_word, 1u);
        if ((__popc(predicted_quote) ^ __popc(real_quote)) & 1u)
            atomicXor(quote_parity_masks + (word_id >> 5u), 1u << (word_id & 31u));
        structural_bitmap[word_id] = ~fullPrefixXorWord(real_quote) & operator_bitmap[word_id];
    }
}

__global__ void validateAndRecoverInStringKernel(const uint32_t* quote_parity_masks,
    const uint8_t* quote_warp_prefix, const uint32_t* operator_bitmap,
    const uint32_t* raw_open_close, uint32_t* structural_bitmap,
    uint32_t* filtered_open_close, uint32_t* structural_counts,
    uint32_t* open_close_counts) {
    uint32_t word_id = blockIdx.x * blockDim.x + threadIdx.x;
    uint32_t lane = threadIdx.x & 31u;
    uint32_t warp_id = word_id >> 5u;
    uint32_t parity_mask = quote_parity_masks[warp_id];
    uint32_t lower_lanes = lane == 0u ? 0u : ((1u << lane) - 1u);
    uint32_t incoming = static_cast<uint32_t>(quote_warp_prefix[warp_id]) ^
        static_cast<uint32_t>(__popc(parity_mask & lower_lanes) & 1u);
    uint32_t correction_mask = 0u - incoming;
    uint32_t structural = structural_bitmap[word_id] ^
        (operator_bitmap[word_id] & correction_mask);
...
static cudaError_t launchHierarchical(HierarchicalInput& input) {
    auto quote_warp_parity = thrust::make_transform_iterator(
        thrust::make_counting_iterator(0u), QuoteParityInput{input.quote_parity_masks});
    cudaError_t status = cudaMemsetAsync(input.recovery_count, 0, sizeof(uint32_t), input.stream);
    if (status != cudaSuccess) return status;
    speculateKernel<<<input.blocks, FULL_THREADS, 0, input.stream>>>(input.input,
        input.input_size, input.operators, input.raw_open_close, input.structural,
        input.trailing_slashes, input.quote_parity_masks, input.utf_error);

    // Level 1: resolve the slash carry and validate the state-zero prediction
    // together, then recover only the failed words.
    lookbackAndValidateEscapedQuotesKernel<<<input.blocks, FULL_THREADS, 0, input.stream>>>(
        input.input, input.trailing_slashes, input.word_count,
        input.recovery_words, input.recovery_count);
    recoverEscapedQuotesKernel<<<input.recovery_blocks, FULL_THREADS, 0, input.stream>>>(
        input.input, input.input_size, input.recovery_words, input.recovery_count,
        input.operators, input.structural, input.quote_parity_masks);

    // Level 2: resolve quote parity, then validate and repair the speculative
    // in-string and final structural bitmaps.
    status = cub::DeviceScan::ExclusiveScan(input.scan_storage, input.scan_bytes,
        quote_warp_parity, input.quote_warp_prefix, QuoteParityXor{},
        static_cast<uint8_t>(0u), input.padded_warps, input.stream);
    if (status != cudaSuccess) return status;
    validateAndRecoverInStringKernel<<<input.blocks, FULL_THREADS, 0, input.stream>>>(
...
int main(int argc, char** argv) {
    ...
    std::vector<HierarchicalInput> inputs(static_cast<size_t>(argc - 2));
    uint64_t total_blocks = 0u;
    for (int argument = 2; argument < argc; ++argument) {
        HierarchicalInput& input = inputs[static_cast<size_t>(argument - 2)];
        ...
        input.recovery_blocks = std::max(1u, std::min(input.blocks, 192u));
        ...
        CUDA_CHECK(cudaMalloc(&input.recovery_words, bitmap_bytes));
        CUDA_CHECK(cudaMalloc(&input.recovery_count, sizeof(uint32_t)));
        ...
        auto quote_warp_parity = thrust::make_transform_iterator(
            thrust::make_counting_iterator(0u), QuoteParityInput{input.quote_parity_masks});
        size_t quote_scan_bytes = 0u;
        CUDA_CHECK(cub::DeviceScan::ExclusiveScan(nullptr, quote_scan_bytes,
            quote_warp_parity, input.quote_warp_prefix, QuoteParityXor{},
            static_cast<uint8_t>(0u), input.padded_warps, input.stream));
        input.scan_bytes = quote_scan_bytes;
        CUDA_CHECK(cudaMalloc(&input.scan_storage, input.scan_bytes));
    }
    CUDA_CHECK(cudaDeviceSynchronize());

    for (int warmup = 0; warmup < 3; ++warmup) {
        for (HierarchicalInput& input : inputs) CUDA_CHECK(launchHierarchical(input));
        CUDA_CHECK(cudaDeviceSynchronize());
    }
    ...
    for (int run = 0; run < runs; ++run) {
        ...
        for (size_t index = 0u; index < inputs.size(); ++index) {
            HierarchicalInput& input = inputs[index];
            CUDA_CHECK(cudaStreamWaitEvent(input.stream, start));
            CUDA_CHECK(launchHierarchical(input));
    ...
    std::vector<uint32_t> recovery_counts(inputs.size()), utf_errors(inputs.size());
    for (size_t index = 0u; index < inputs.size(); ++index) {
        HierarchicalInput& input = inputs[index];
        ...
        CUDA_CHECK(cudaMemcpy(open_close.data(), input.filtered_open_close,
                              input.word_count * sizeof(uint32_t), cudaMemcpyDeviceToHost));
        CUDA_CHECK(cudaMemcpy(&recovery_counts[index], input.recovery_count,
    ...
    std::cout << std::fixed << std::setprecision(3)
              << "method=cujson_hierarchical_speculation inputs=" << inputs.size()
    ...
    for (size_t index = 0u; index < inputs.size(); ++index)
        std::cout << "input=" << index << " bytes=" << inputs[index].input_size
                  ...
                  << " recoveries=" << recovery_counts[index]
    ...
    for (HierarchicalInput& input : inputs) {
        ...
        CUDA_CHECK(cudaFree(input.recovery_words));
        CUDA_CHECK(cudaFree(input.recovery_count));
